...
BOOST_AUTO_TEST_CASE (testCopyHeap) {
    fHeap<long, long> heap1;
    heap1.enqueue(0,10);
    heap1.enqueue(1,20);
    heap1.enqueue(2,5);
    fHeap<long, long> copy;
    copy = heap1;
    BOOST_CHECK_EQUAL(copy.size(), 3);
    BOOST_CHECK_EQUAL(copy.getTopIdx(), 2);
    copy.dequeue();
    BOOST_CHECK_EQUAL(copy.getTopIdx(), 0);
    copy.dequeue();
    BOOST_CHECK_EQUAL(copy.getTopIdx(), 1);
}