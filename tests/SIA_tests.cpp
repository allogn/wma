...
BOOST_AUTO_TEST_CASE (dijsktra_simple_test) {
    //primitive graph
    Matcher<int,int,int> M(2);
    igraph_add_edge(&M.graph, 0, 1);
    ...
    M.new_edges.resize(2); //dummy for Dijkstra, must contain non-existing edges
    for (int i = 0; i < 2; i++) M.new_edges[i].exists = false;
    M.iteration_init(0);
    ...
    igraph_shortest_paths_dijkstra(&M.graph, &correct_answer, igraph_vss_1(0), igraph_vss_all(), &real_weight, IGRAPH_OUT);
    BOOST_CHECK(MATRIX(correct_answer, 0, 0) == 0);
    BOOST_CHECK(MATRIX(correct_answer, 0, 1) == 10);
    igraph_shortest_paths_dijkstra(&M.graph, &correct_answer, igraph_vss_1(1), igraph_vss_all(), &real_weight, IGRAPH_OUT);
