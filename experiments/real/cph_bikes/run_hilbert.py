...
for f in os.listdir(path.join(data_path,exproot,expdir)):
    ...
    for facilities in range(200,600,30):
        p = subprocess.Popen([path.join(root_path,'bin','hilbertsolver'),'-i',
			  ...
			  '-c',"9",
			  '-n',str(facilities),
		   	  '-f',path.join(data_path,exproot, expdir, "facility_location_hours.csv"),
			  '-o',path.join(data_path,exproot,expdir,'solutions','hilbert',id + "fac" + str(facilities) + ".json")],
