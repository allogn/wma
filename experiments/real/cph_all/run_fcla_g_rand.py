...
for f in os.listdir(path.join(data_path,'real',exproot,expdir)):
    ...
    for facilities in range(30,80,5):
   	 p = subprocess.Popen([path.join(root_path,'bin','fcla'),'-i',
				  ...
				  '-n',str(facilities),
				  '-g','1',
				  '-f',path.join(data_path,'real',exproot, expdir, "facility_location_hours_full.csv"),
				  '-o',path.join(data_path,'real',exproot, expdir,'solutions','fcla_g_rand',id + "fac" + str(facilities) + ".json")],
