...
storage_path = '/q/storage/alogins/fcla_data/geometric_with_coords/var_pot_fac_loc'
...
for number_pot_fac in range(500,2000,200):
    new_filename = str(number_pot_fac) + ".csv"
    outfile = path.join(storage_path, "facilities", new_filename)
    facilities = np.random.choice([n for n in network.G.nodes() if n not in network.sources], number_pot_fac)
    capacity = 50000
