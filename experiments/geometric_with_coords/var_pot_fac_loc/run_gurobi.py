...
expdir = 'var_pot_fac_loc'
exproot = 'geometric_with_coords'
...
for f in os.listdir(path.join(data_path,exproot,expdir)):
    ...
    for facility_file in os.listdir(path.join(data_path,'geometric_with_coords',expdir,'facilities')):
        ...
        p = subprocess.Popen(['python', path.join(root_path,'scripts','solveGurobi.py'),
				  ...
				  str(capacity),
				  str(facilities),
				  path.join(data_path, exproot,expdir,'solutions','gurobi',id + "fac" + str(facility_file) + ".json"),
				  path.join(data_path, exproot,expdir, full_fac_file),
				  path.join(data_path, exproot,expdir, 'distMtx.pkl')],
