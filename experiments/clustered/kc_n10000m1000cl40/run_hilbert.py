...
for f in os.listdir(path.join(data_path,'clustered',expdir)):
    ...
    for facilities in range(200, 700, 100):
        print("facilities " + str(facilities))
        capacity = 2000/facilities+1
    		  
        p = subprocess.Popen([path.join(root_path,'bin','hilbertsolver'),'-i',
	     		     full_target_path,
			     '-c',str(capacity),
			     '-n',str(facilities),
			     '-o',path.join(data_path,'clustered',expdir,'solutions','hilbert',id + "_" + str(facilities) + ".json")],
